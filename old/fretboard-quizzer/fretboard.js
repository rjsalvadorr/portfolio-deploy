...
var FretboardQuizzer = {
    ...
    /////   HTML GENERATION   /////////////////////////////////////////////////
    ///////////////////////////////////////////////////////////////////////////
    getStringUnit: function(fretNumber, stringNumber) {
        var stringCoordinates = stringNumber + ',' + fretNumber;
        var stringClass = stringNumber <= (this.Constants.NUM_STRINGS / 2) ? "string-treble" : "string-bass";
        return '<div class=\"string-unit\" data-coordinates=\"' + stringCoordinates + '\"><div class=\"' + stringClass + '\"></div></div>';
    },

    getFretUnit: function(fretNumber, stringNumber) {
        var stringClass = stringNumber <= (this.Constants.NUM_STRINGS / 2) ? "string-treble" : "string-bass";
        return '<div class=\"fret-unit\"><div class=\"' + stringClass + '\"></div></div>';
    },
    ...
    getNut: function() {
        ...
        for(var stringNum = 1; stringNum <= this.Constants.NUM_STRINGS; stringNum++) {
            nutHtml += this.getStringUnit(fretNumber, stringNum);
        }
        nutHtml += '</div>';
        return nutHtml;
    },
    ...
    getFretboardUnit: function(fretNumber, lengthPx) {
        ...
        for(var stringNum = 1; stringNum <= this.Constants.NUM_STRINGS; stringNum++) {
            fretboardUnitHtml += this.getStringUnit(fretNumber, stringNum);
        }
    ...
    clearMarkingOnString: function(stringNum) {
        var selectorString = '.string-unit[data-coordinates^=\'' + stringNum + '\'] > .note-marker';
    ...
    markNote: function(stringUnitElement) {
        ...
        var markingJqObj = $('.string-unit[data-coordinates=\'' + stringNum + ',' + fretNum +'\'] > .note-marker');
        if(markingJqObj.length == 0) {
            if(this.markingExistsForString(stringNum)) {
                this.clearMarkingOnString(stringNum);
            }
            $(stringUnitElement).append('<div class=\'note-marker\'></div>');
    ...
    resetQuiz: function() {
        ...
        var imgSrc =  "img-notes/" + this.currentQuizNote.name + '.png';
